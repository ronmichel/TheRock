...
def main():
    ...
    parser.add_argument(
        "--amdgpu_family",
        required=True,
        help="Subamdgpu_family inside the CloudFront distribution.",
    )
    parser.add_argument(
        "--dir",
        required=True,
    ...
    try:
        ...
        log.error(
            'Error: --list_whls must be a valid Python list, e.g. \'["jax_pjrt.whl","jax_plugin.whl","jaxlib.whl"]\''
        )
        exit(1)
    ...
    download_files(
        args.cloudfront_url, args.amdgpu_family, whl_list, args.wheelhouse_dir
    )
