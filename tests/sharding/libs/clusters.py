...
class Cluster(object):
    ...
    def installPreReq(self):
        if not all(
            self.installPkgs(
                "wget", "python3", "python3-pip", "libgfortran5", updateCache=True
            )
        ):
            return False
        ...
        self.runCmd(
            "gpg", "--dearmor", "-o", f"/usr/share/keyrings/{keyfile}", stdin=content
        )
        self.writeFile(
...
class KubeCluster(Cluster):
    ...
    def _waitForNodes(self, replicas, phase="Running", timeout=5 * 60):
        for i in range(int(timeout / 5)):
            pods = self._listPods()
            if (
                len(tuple(filter(lambda pod: pod.status.phase == phase, pods)))
                == replicas
            ):
                return pods
            time.sleep(5)
    ...
    def _setNodes(self, *pods):
        if not self.nodes:
            self.nodes = [
                nodes.KubePod(self, pod.metadata.name)
                for pod in (pods or self._getPods())
            ]
        return self.nodes
