		...
		try:
			with open(cacheFile, 'r') as fd:
				try:
					...
					# If locking fails, still try to read the file
					content = fd.read()
			...
			log(f"Error reading cache file {cacheFile}: {e}")
			return {}
		...
		try:
			with open(cacheFile, 'w') as fd:
				try:
					...
					# If locking fails, still write the file
					fd.write('\n'.join([f'{test}|{duration}' for (test, duration) in cache.items()]))
		except Exception as e:
			log(f"Error updating cache file {cacheFile}: {e}")
